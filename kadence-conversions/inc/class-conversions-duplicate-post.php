 ...
 */
/**
 * Class Kadence_Conversions\Duplicate_Post
 *
 * @package Kadence Conversions
 */

namespace Kadence_Conversions;
...
/**
 * Kadence_Conversions_Duplicate_PostClass.
 */
class Duplicate_Post {
	...
	public function dupe_link( $actions, $post ) {
		...
		if ( ! current_user_can( apply_filters( 'kadence_conversions_duplicate_post_capability', 'edit_kadence_conversions' ) ) ) {
			return;
		}
		
		$actions['duplicate'] = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=' . $this->slug . '&action=kadence_duplicate_' . $this->slug . '&amp;post=' . $post->ID ), 'kadence_duplicate_post_' . $post->ID ) . '" aria-label="' . esc_attr__( 'Make a duplicate from this.', 'kadence-conversions' )
			. '" rel="permalink">' . esc_html__( 'Duplicate', 'kadence-conversions' ) . '</a>';
	...
	public function duplicate_action() {
		if ( empty( $_REQUEST['post'] ) ) {
			wp_die( esc_html__( 'No post to duplicate has been supplied!', 'kadence-conversions' ) );
		}
		...
		if ( false === $clone_post ) {
			...
			wp_die( sprintf( esc_html__( 'Post creation failed, could not find original post: %s', 'kadence-conversions' ), esc_html( $post_id ) ) );
		}
	...
	public function duplicate( $post ) {
		...
		$title              = sprintf( esc_html__( '%s (Copy)', 'kadence-conversions' ), $post->post_title );
		...
		if ( $new_post_id !== 0 && ! is_wp_error( $new_post_id ) ) {
			...
			do_action( 'kadence_conversions_duplicate_post', $new_post_id, $post );
		}
	...
	public function duplicate_meta_info( $new_id, $post ) {
		 ...
		 * @return array
		 */
		$meta_blacklist = apply_filters( 'kadence_conversions_duplicate_post_exclude_list', $meta_blacklist );
		 ...
		 * @return array
		 */
		$meta_keys = apply_filters( 'kadence_conversions_duplicate_meta_keys', $meta_keys );
